            ...
            for (int r = 0; r < stackedGlasses.Count - 1; r++) // Ignore last row(floor)
                for (int g = 0; g < stackedGlasses[r].Length; g++)
                    if (stackedGlasses[r][g].Numerator > 0 && stackedGlasses[r][g].Numerator < stackedGlasses[r][g].Denominator) // Glass is not empty or full
                    {
                        ...
                        // If the glass being monitored is expected to reach full capacity through the optimization operation, reduce the number of water additions by one
                        // We want to do the last pouring of water in the simulation step
                        if (r == rad -1 && g == glas - 1 &&
                            temp * addTable[r][g] == stackedGlasses[r][g].Denominator)
                                timesUntilFull--;
